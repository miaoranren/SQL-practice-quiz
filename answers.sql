...
SELECT common_name, price FROM melons WHERE price > 5 and melon_type like 'Watermelon';
...
SELECT email FROM salespeople WHERE region LIKE 'Southwest' or region LIKE 'Northwest';
...
SELECT orders.id, status, order_total FROM orders JOIN customers ON (orders.customer_id = customers.id) WHERE email LIKE 'pclark74@gmail.com';
